...
def KQBC(X_train, Y_train, T, kernel, queries=100, **kwargs):
    ...
    for ii in np.arange(1, samp_num):
        ...
        # try:
        # polytope = Polytope(A=restri, b=np.zeros(restri.shape[0]))
        #
        # try:
        #     hitandrun = HitAndRun(polytope=polytope, starting_point=co1.squeeze())
        # except:
        #     print('oh no')
        # co2 = hitandrun.get_samples(n_samples=T)[T - 1]
        #
        # if ii == 1:
        #     hitandrun = HitAndRun(polytope=polytope, starting_point=co1.squeeze())
        # else:
        #     try:
        #         hitandrun = HitAndRun(polytope=polytope, starting_point=co2.squeeze())
        #     except:
        #         print('oh no')
        # co1 = hitandrun.get_samples(n_samples=T)[T - 1]
        ...
        #     q_cnt += 1
            if (Y_train[ii] * pred1 >= 0):
                # coef[extension] = np.expand_dims(np.matmul(A, co1.T), axis=-1)
                coef[extension] = np.matmul(A, co1)
            else:
                # coef[extension] = np.expand_dims(np.matmul(A, co2.T), axis=-1)
